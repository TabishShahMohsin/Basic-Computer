# Basic-Computer
* Implementation of everything taught in [DLSD(COC2072)](/University/Syllabus.pdf#page=16) + [Computer Architecture(COC2082)](/University/Syllabus.pdf#page=19)
* [comptuer-8bits](https://github.com/leonicolas/computer-8bits) : An excellent resource
* Some university provided books that this basic computer is based on: 
    * Computer System Architecture - M. Morris Mano
    * Digital Logic and Computer Design - M. Morris Mano

* Not taught but needed: For looped circuits (like Q = B + Q), the sim can't find the correct state, hence RESET pin is used to make inital Q as 0. Same as saying that a recursive function needs a base state to make better sense.
    * Convention: R=0 should reset with Q=0
...
### GPT provided structure:
```text
├── .gitignore
├── README.md
└── src/
    ├── 01-DLSD Circuits/    # The building blocks for the ALU and Bus
    ├── 02-combinational/    # The building blocks for the ALU and Bus
    │   ├── HalfAdder.circ
    │   ├── FullAdder.circ
    │   ├── 4BitAdder.circ
    │   ├── 16BitAdder.circ
    ...
    │
    ├── 03-sequential/       # Memory elements and counters
    │   ├── D_FlipFlop.circ
    │   ├── JK_FlipFlop.circ
    │   ├── T_FlipFlop.circ
    ...
    │
    ├── 04-alu/              # Arithmetic Logic Shift Unit (ALSU)
    ...
    │
    ├── 05-memory/           # Main Memory (RAM)
    │   └── RAM_4096x16.circ     # Usually wraps Logisim's built-in RAM with Mano's Read/Write pins
    │
    ├── 06-control-unit/     # The "brain" decoding instructions and timing
...
### Some good to remember Logisim shortcuts: 😅
...
| | Zoom Out | `Ctrl` + `-` | `Cmd` + `-` |
| | Reset Zoom / Zoom to Fit | `Ctrl` + `0` | `Cmd` + `0` |
| **Canvas** | Change Component Facing | `Arrow Keys` (↑, ↓, ←, →) | `Arrow Keys` (↑, ↓, ←, →) |
| | Delete Component | `Delete` / `Backspace` | `Delete` |
